...
def solve(symbols):
    ...
    val = (int(symbols[0]) if isinstance(symbols[0], str) else solve(symbols[0]))
    
    for i in range(2, len(symbols), 2):
        factor = (int(symbols[i]) if isinstance(symbols[i], str) else solve(symbols[i]))
        if symbols[i-1] == "*":
            val *= factor 
        else:
            val += factor
...
def main():
    equations = [parse_nested(l) for l in utils.get_lines(18)]

    print("PART 1:")
    print(sum(solve(equation) for equation in equations))

    print("PART 2:")
    print(sum(solve_precedence(equation) for equation in equations))
